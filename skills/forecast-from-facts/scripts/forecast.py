...
为什么要好几个方法而不是一个:
    ...
    杰瑞 Q4 是交付旺季,期末存货和下季营收的关系被季节性彻底盖住。
...
def m_inventory(qs, target):
    ...
    而且这笔钱**已经花出去了** —— 是行为不是说法,无法被管理层口径影响。
...
def chain_position(qs, capyoy: dict) -> dict | None:
    ...
    trend = ("在得份额" if last > first * 1.15 else
             "在丢份额" if last < first * 0.85 else "基本跟着行业走")
    return {"points": pts, "近四季首": first, "近四季末": last, "判断": trend}
