...
**每跑一次自动落一份结构化快照** `data/snapshots/<代码>/<日期>.json`,
下次再跑时第 0 节自动出「与上次对比」——净利同比掉了几个百分点、
毛利率离判据线还有多远、股东户数增减多少。`--no-diff` 跳过。

`--html <文件>` 同时出一份带版式的 HTML(浏览器可看,也能
`node html2pdf.mjs` 打成 PDF)。**HTML 给人看,JSON 给机器 diff** ——
两份 HTML 做对比只会得到排版噪音,真正要比的是结构化的数。

新增三个筹码类子命令(2026-08-28):`efdata hksc <代码>` 港资多期持股序列(比十大股东
单期长得多)、`efdata northbound <代码> [--daily]` 北向个股持仓、`efdata ann <代码>` 公司公告。
前两个已接进 `preport` 第 4 层。

⚠ **港资判方向要看股数不看比例**:持股数增加但占总股本比例下降,那是股本摊薄
(增发/转股/激励授予),**不是减仓**。只看比例会看反,preport 会自动提示这种情况。

⚠ 北向给的是**持仓**不是**净买入**。2024-08-19 起交易所不再公布单票日度净买入 ——
监管改了披露规则,不是接口坏了,没有免费替代。

`efdata` 是 efinance 的封装,**18 个稳定子命令 + 6 个本机取不到的**(失败会打印替代方案,
退出码 2)。`efdata --list` 看全部,`efdata --check` 探测当前可用性。
每个子命令拿到什么(含真实返回列)见 `docs/04-efdata数据全集.md`。
