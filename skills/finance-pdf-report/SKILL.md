...
description: >
  ...
  **本 skill 带 `deep_report.py`** —— 把 preport 的快照 + journal 的判断
  渲染成给人看的深度分析报告(结论前置 / 正反证据 / 价格坐标图 / 三情景赔率图 /
  回撤容忍线 / 数据来源表),不联网、不发明判断。
  触发词:金融 PDF / 投资报告 PDF / 分析报告 PDF / 出 PDF / 报告版式 /
  持仓报告 / 研报格式 / html 转 pdf / playwright pdf / md2pdf 黑白 /
  报告要好看 / 带图表的 PDF / 深度报告 / deep_report / dreport /
  给人看的报告 / 结论前置 / 赔率图 / 价格坐标图。
...
## 〇　两类报告:数据层 vs 结论层（**先分清再动手**）

2026-08-28 被问「为什么只有一只票有投资分析报告」才发现少了一层。
四只票明明都出过 HTML，但那是**机器报告**，读者要的是**深度分析**。
两者不是精简版与完整版的关系，是**两种东西**:

| | `preport --html`（数据层） | `dreport`（结论层） |
|---|---|---|
| 标题 | 持仓**决策报告** | 投资分析**报告** |
| 结构 | 0a~9 十节数据表 | 结论前置 → 三档操作 → 正反证据 → 完备性 → 来源 |
| 结论 | 只有判据表（条件 → 动作） | 一句话前置 + 具体价位 |
| 论证 | 无 | 正反证据分组，每条 = 数 + 判读 + **判读规则** |
| 图 | 0 | 价格坐标 · 三情景赔率（带回撤容忍线）· 同业对照 |
| 输入 | 代码 + 成本，**要联网取数** | 快照 + journal，**不联网** |
| 错了是谁的错 | 接口的（数据错） | 判断的（thesis 错） |

**最后一行是分成两个工具的真正原因。**混在一起，读者分不清哪句是数据、
哪句是观点;而复核的时候，「数据错了」和「我想错了」要走完全不同的改法。

```bash
preport 300308:1100.00           # ① 先跑数据层 → 自动落快照
journal log 300308 --price ... --bear-px 700 --base-px 950 --bull-px 1200 ...
                                  # ② 记下判断（三情景**价位**必须记，图靠它）
dreport 300308 --tolerance 30 --out r.html    # ③ 渲染成给人看的
```

### 硬约束（有闸:`scripts/check_report_contract.py`）

1. **不发明判断。**结论 / thesis / 可证伪判据全部来自 journal 那条记录 ——
   也就是**将来要被复核的那份**。没记 journal 的票出不了结论段，这是有意的。
2. **一份报告只能有一个分母。**决策当日价与快照当日价不同时，正文一律按快照价算，
   并在抬头写明。两个分母并排出现，读者看不出来但结论会差好几个百分点。
3. **赛道判为不适用的指标，不准进正/反面证据**，只能进「中性项与口径提醒」并带 ⚠️。
4. **完备性表里「⊘ 不适用」和「❌ 缺」不能混。**「缺」会让人去补一个本来就不该用的
   指标，补回来只会让结论更错。
5. **每张图必须有 figcaption**;正文不准出现没渲染的裸 `**`。
6. **回撤容忍线不给就不画** —— 那个数因人而异，没有合理默认值。

### 一条实测的教训

上面第 5 条是闸抓出来的，不是我看出来的。5 份报告我都渲染成图逐页看过，
**仍然漏掉了正文里 5 处没渲染的 `**粗体**`** —— 字太小，眼睛扫过去了。
渲染自检抓大问题（布局、溢出、图糊），机器闸抓小而致命的（口径、边界、标记）。
两个都要，谁也替代不了谁。
