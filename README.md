...
| | 是什么 | 落在哪 |
|---|---|---|
| **先判赛道**（硬规范） | 拿到一只票**先判赛道 → 定数据源 → 再跑策略**。反过来就是跑了错的策略。`preport` 自己会先判（第 0a 节），并把该赛道**失效的指标逐个打标** —— 周期股的 PEG 会带 ⚠️，基准率整节禁用 | `sectors` · 闸 `check_sector_contract.py` |
...
0a 赛道与适用性 —— **先判赛道，标出哪些指标在这里会给假信号**
0  与上次对比 —— 有历史快照时自动插到最前面
```

**第 0a 节是硬规范不是可选项。**同一套判据套所有赛道会出错：拿 AI 算力链
那套跑油气设服票，云厂 Capex 无关、PEG 算出 −15.30、基准率不适用 ——
**不是工具坏了，是用错了框架**。现在这些指标会被逐个打 ⚠️，基准率整节禁用。

| | 成长股 | 周期股 |
|---|---|---|
| 看什么 | 增速**持续性** | **周期位置** |
| PEG | 有意义 | 低谷给「便宜」假信号、高峰给「贵」假信号 |
...
七道：零个人信息 · `_lab_root` 多份一致 · 判据线==基准率线 ·
**先判赛道再跑策略** · 软链无断链 · py 语法 · sh 语法。
