...
| `finance-pdf-report` | 出金融 PDF / 深度报告的固定版式（结论前置、语义色、内联 SVG、强制数据来源表），含 `dreport` | 不管内容写什么 |
...
preport 300308:1100 --html r.html # 同上，带版式的 HTML（数据层，机器生成）
dreport 300308 --tolerance 30 --out r.html  # ★★ 给人看的深度分析报告（结论层）
...
**这是两种东西，不是精简版和完整版。**`preport` 出的是数据层，
每个数有出处、可复核，但它**不替你下判断**；`dreport` 出的是结论层，
读快照 + journal，渲染成人真正会读的那份：

| | `preport --html` | `dreport` |
|---|---|---|
| 结构 | 0a~9 十节数据表 | 结论前置 → 三档操作 → 正反证据 → 完备性 → 来源 |
| 结论 | 只有判据表（条件 → 动作） | 一句话前置 + 具体价位 |
| 论证 | 无 | 正反证据分组，每条 = 数 + 判读 + **判读规则** |
| 图 | 0 | 价格坐标 · 三情景赔率（带你自己的回撤容忍线）· 同业对照 |
| 输入 | 代码 + 成本，**要联网** | 快照 + journal，**不联网** |
| 错了是谁的错 | 接口的（数据错） | 判断的（thesis 错） |

最后一行是分成两个工具的真正原因 —— 复核时「数据错了」和「我想错了」
要走完全不同的改法，混在一份文件里就分不开了。

**`dreport` 不发明判断。**结论、三情景、可证伪判据全部来自 `journal` 那条记录，
也就是**将来要被复核的那份**。没记 journal 的票出不了结论段，这是有意的。

三情景赔率图上可以画一条**你自己的回撤容忍线**（`--tolerance 30`）——
这让「EV 是正的但我仍该减仓」从一段话变成一张图：熊市那根穿过虚线多少，一眼可见。
不给这个参数就不画，因为那个数因人而异，没有合理默认值。
...
八道：零个人信息 · `_lab_root` 多份一致 · 判据线==基准率线 ·
**先判赛道再跑策略** · **深度报告层合约** · 软链无断链 · py 语法 · sh 语法。

那道「深度报告层合约」查的是**静默错法**：EV 与三情景价位对不上、
把赛道判为不适用的指标当证据用、完备性表把「不适用」标成「缺」、
图没有说明、粗体没渲染。这些错**都不会报错**，报告照样生成、照样好看。
实测:5 份报告我渲染成图逐页看过，仍漏掉正文里 5 处没渲染的粗体标记 ——
**渲染自检抓大问题，机器闸抓小而致命的**，谁也替代不了谁。
